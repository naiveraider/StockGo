    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40" onClick={onClose}>
      <div className="w-full max-w-md rounded-lg border border-slate-200 bg-white shadow-xl" onClick={(e) => e.stopPropagation()}>
        ...
        <div className="max-h-72 overflow-y-auto">
          {isLoading && <div className="px-4 py-3 text-sm text-slate-500">Loading...</div>}
          {!shouldFetch && <div className="px-4 py-3 text-sm text-slate-500">Type to search by symbol or company name.</div>}
          {shouldFetch && !isLoading && results.length === 0 && <div className="px-4 py-3 text-sm text-slate-500">No matches.</div>}
          {results.map((s) => (
...
export default function LongTermPage() {
  ...
  const url = `${API_BASE}/v1/screener/long-term?${queryParams}`;
  const { data, error, isLoading, mutate } = useSWR<LongTermPage>(
    access === "ok" && token ? url : null,
    (u: string) => authedJson<LongTermPage>(u, token as string)
  );
    ...
    <div className="space-y-4">
      <div className="rounded-lg border border-slate-200 bg-white p-4">
        <div className="flex flex-wrap items-center justify-between gap-4">
          <div>
            <h2 className="text-lg font-semibold text-slate-900">Long-term bias</h2>
            <p className="text-sm text-slate-500">Stocks in your long-term bias list.</p>
          </div>
      ...
      <div className="overflow-hidden rounded-lg border border-slate-200 bg-white">
        <table className="w-full text-sm">
          ...
          <tbody>
            {items.map((it) => {
              const biasColor = it.bias === "UP" ? "text-emerald-600" : it.bias === "DOWN" ? "text-rose-600" : "text-amber-600";
              return (
                <tr key={it.ticker} className="border-t border-slate-100 hover:bg-slate-50">
                  <td className="px-4 py-3 font-medium text-slate-900">
                    <Link className="text-yahooBlue hover:underline" href={`/quote/${it.ticker}/long-term`}>
                      {it.ticker}
                    </Link>
                  </td>
            ...
            {items.length === 0 && !isLoading && (
              <tr>
                <td className="px-4 py-6 text-slate-500" colSpan={5}>
                  {searchQuery ? "No matches. Try a different search or clear the filter." : "No stocks in your long-term bias list yet."}
                </td>
              </tr>
            )}
          </tbody>
      ...
      <SearchDialog
        ...
        onSelect={(ticker) => {
          setSearchOpen(false);
          router.push(`/quote/${ticker}/long-term`);
        }}
        onAdd={async (ticker) => {
          if (!token) {
            router.push("/login");
          ...
          await fetch(`${API_BASE}/v1/screener/add?ticker=${encodeURIComponent(ticker)}&bucket=long`, {
            method: "POST",
